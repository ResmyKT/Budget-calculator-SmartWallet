...
welcomeuser.innerHTML=`<h4><i class="fa-solid fa-user fa-fade"></i> Welcome ${loggedkey}</h4>`
...
function register()
{
    ...
    if(email==''||uname=='' ||password=='')
    {
        ...
        if(email in localStorage){
            ...
            alert("User Registerd Succeessfully")
...
function displayincomeArray(){
    ...
    for(i of incomearray){
        incomedetails.innerHTML+=`
        <tr style="border-bottom:2px blue solid">
        <td>${i.type}</td>
        <td>${i.amt}</td>
        <td>${i.bal}</td>
        <td>${i.dt}</td>
